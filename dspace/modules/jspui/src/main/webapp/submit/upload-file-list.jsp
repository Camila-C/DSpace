...
<dspace:layout style="submission" locbar="off" navbar="off" titlekey="jsp.submit.upload-file-list.title">
  ...
  <form action="<%= request.getContextPath() %>/submit" method="post" onkeydown="return disableEnterKey(event);">
    ...
    <%	if (justUploaded) { %>
		<h1>
			<fmt:message key="jsp.submit.upload-file-list.heading1"/>
			<dspace:popup page="<%= LocaleSupport.getLocalizedMessage(pageContext, \"help.index\") + \"#uploadedfile\"%>">
    ...
    <table class="table table-bordered" align="center" summary="Table dispalying your submitted files">
      <tr>
		...
		%>
    	    <div class="row">
						<input class="btn btn-success col-md-2 col-md-offset-5" type="submit" name="submit_more" value="<fmt:message key="jsp.submit.upload-file-list.button4"/>" />
					</div>
		<%	} %>
		<br/>
		<%	//Show information about how to verify correct upload, but not in workflow mode!
    		if (allowFileEditing) {
		%>
				<div class="alert alert-info">
					<fmt:message key="jsp.submit.upload-file-list.info7"/>
				</div>
          ...
          <li class="uploadHelp"><fmt:message key="jsp.submit.upload-file-list.info4"/></li>
            ...
            <li class="uploadHelp">
            	...
            	<dspace:popup page="<%= LocaleSupport.getLocalizedMessage(pageContext, \"help.index\") + \"#checksum\"%>">
								<fmt:message key="jsp.submit.upload-file-list.help1"/>
							</dspace:popup>
						</li>
					<%	} else { %>
            <li class="uploadHelp">
