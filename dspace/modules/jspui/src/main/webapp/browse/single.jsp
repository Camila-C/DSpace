...
<dspace:layout titlekey="browse.page-title">
	...
	<div id="browse_navigation" class="well text-center">
		<form method="get" action="<%= formaction %>">
			<%--@declare id="starts_with"--%><input type="hidden" name="type" value="<%= bix.getName() %>"/>
	...
	%>
			...
			<input type="submit" class="btn btn btn-unrn-reverse" value="<fmt:message key="browse.nav.go"/>" />
			<br/>
	...
	<%
				for (char c = 'A'; c <= 'Z'; c++) {
	%>
			<a lass="label label-default" href="<%= sharedLink %>&amp;starts_with=<%= c %>"><%= c %></a>
	<%
				}
	%>
			<br/>
	...
	<div id="browse_controls" class="well text-center">
		<form method="get" action="<%= formaction %>">
			<%--@declare id="order"--%><%--@declare id="rpp"--%>
	...
	%>
			...
			<input type="submit"
				   class="btn btn btn-unrn-reverse"
				   name="submit_browse"
	...
	<%
						if (results[i][1] != null) {
	%>
								&amp;authority=<%= URLEncoder.encode(results[i][1], "UTF-8") %>"
						   class="authority <%= bix.getName() %>"
						><%= Utils.addEntities(results[i][0]) %></a>
	<%
						} else {
	%>
								&amp;value=<%= URLEncoder.encode(results[i][0], "UTF-8") %>"
						><%= Utils.addEntities(results[i][0]) %></a>
	<%
						}
	%>
